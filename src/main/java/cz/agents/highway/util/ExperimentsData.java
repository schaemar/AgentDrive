...
public class ExperimentsData {
    ...
    public void simulationEnded()
    {
        ...
        {
            ENDTIME = storage.getEventProcessor().getCurrentTime();
        }
    ...
    public void updateNumberOfCars(RadarData object)
    {
        ...
        if(Configurator.getParamBool("highway.dashboard.systemTime",false)) {
            ...
            numberOfCarsInSimulation.add(new Pair<Float, Integer>(storage.getEventProcessor().getCurrentTime() / 1000f, object.getCars().size()));
        }
    }
    public void updateTimesAndGraphOfArrivals(RadarData object,int id)
    {
        ...
        {
            timesOfArrival.add(storage.getEventProcessor().getCurrentTime()/1000f);
            Pair<Float, Float> floatFloatPair = graphOfArrivals.get(id);
            graphOfArrivals.put(id, new Pair<Float, Float>(floatFloatPair.getKey(), storage.getEventProcessor().getCurrentTime() / 1000f));
        }
    }
    public void updateDistances(RadarData object,Map.Entry<Integer, RoadObject> entry)
    {
        ...
        {
            timeKey = storage.getEventProcessor().getCurrentTime();
        }
        //FUNNEL
    ...
    public void vehicleCreation(int id)
    {
        if(Configurator.getParamBool("highway.dashboard.systemTime",false)) {
            ...
            else graphOfArrivals.put(id,new Pair<Float, Float>((System.currentTimeMillis() - storage.getSTARTTIME()) / 1000f,null));
        }
        else
        {
            graphOfArrivals.put(id, new Pair<Float, Float>(storage.getEventProcessor().getCurrentTime()/1000f, null));
        }
    }
}
